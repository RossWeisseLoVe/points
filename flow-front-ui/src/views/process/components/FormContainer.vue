              ...
              if(iframe.contentWindow?.vueObj){
                if(procInstId){
                  getFormDataInfoByProcessInstanceId({procInstId}).then(result => {
                    iframe.contentWindow.CustomForm.loadFormInfo({formJson: res.formJson, editData: JSON.parse(result.formData), editable: false});
                  });
                }else{
                  iframe.contentWindow.CustomForm.loadFormInfo({formJson: res.formJson, editData: {}, editable: true});
                }
              }
